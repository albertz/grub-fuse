		       ...
		       enum grub_fshelp_filetype expecttype)
{
  grub_err_t err;
	...
	find_file_foundtype = GRUB_FSHELP_DIR;
	find_file_iterate_dir = iterate_dir;
	find_file_read_symlink = read_symlink;
	find_file_symlinktest = 0;
	find_file_rootnode = rootnode;
  err = find_file (path, rootnode, foundnode);
  if (err)
    return err;

  /* Check if the node that was found was of the expected type.  */
  if (expecttype == GRUB_FSHELP_REG && find_file_foundtype != expecttype)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE, "not a regular file");
  else if (expecttype == GRUB_FSHELP_DIR && find_file_foundtype != expecttype)
    return grub_error (GRUB_ERR_BAD_FILE_TYPE, "not a directory");
